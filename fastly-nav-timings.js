...
let pageLoadId = Math.floor(Math.random() * 100000000);

// Memory usage stats collected as soon as JS executes, so we can compare the
// delta later on page unload
let jsHeapUsed = window.performance.memory && window.performance.memory.usedJSHeapSize;
...
window.addEventListener("load", function() {
  ...
  setTimeout(function() {
    ...
    let params = pageLoadEvent();
    var queryString = Object.keys(params).map(key => key + '=' + params[key]).join('&');
    navigator.sendBeacon("/.rum-beacon", queryString);
  }, 0);
});
